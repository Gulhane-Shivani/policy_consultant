...
const Renewals = () => {
  const [stats] = useState([
    { label: 'Upcoming Renewals', value: '342', change: '+8.4%', icon: Calendar, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Processed Today', value: '24', change: '+15.2%', icon: RefreshCw, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Overdue Renewals', value: '12', change: '-5.1%', icon: Clock, color: 'text-rose-600', bg: 'bg-rose-50' },
  ]);

  const [data] = useState([
    { id: 'REN-101', client: 'Alice Johnson', policy: 'POL-7721', dueDate: 'In 3 days', status: 'Pending' },
    { id: 'REN-102', client: 'Bob Wilson', policy: 'POL-6612', dueDate: 'Tomorrow', status: 'In Progress' },
    { id: 'REN-103', client: 'Charlie Davis', policy: 'POL-5503', dueDate: 'Expired', status: 'Overdue' },
    { id: 'REN-104', client: 'Diana Prince', policy: 'POL-4494', dueDate: 'In 1 week', status: 'Pending' },
  ]);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 font-bold">Monitor and process upcoming policy renewals.</p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-6 font-bold text-slate-900">{item.client}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-500">{item.policy}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-900">{item.dueDate}</td>
