...
import { 
  LifeBuoy, FileSearch, AlertTriangle,
...
const ClaimsSupport = () => {
  const [stats] = useState([
    { label: 'Active Claims', value: '124', change: '+8.4%', icon: LifeBuoy, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Under Review', value: '42', change: '+12.5%', icon: FileSearch, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Urgent Cases', value: '12', change: '-2.1%', icon: AlertTriangle, color: 'text-rose-600', bg: 'bg-rose-50' },
  ]);

  const [data] = useState([
    { id: 'CLM-901', client: 'John Doe', type: 'Car Accident', amount: '$4,200', status: 'Pending' },
    { id: 'CLM-902', client: 'Jane Smith', type: 'Medical', amount: '$850', status: 'In Progress' },
    { id: 'CLM-903', client: 'Robert Brown', type: 'Property', amount: '$15,000', status: 'Approved' },
    { id: 'CLM-904', client: 'Emily Davis', type: 'Life', amount: '$50,000', status: 'Denied' },
  ]);

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-black text-slate-900 tracking-tighter">Claims Support</h1>
        <p className="text-slate-500 font-bold">Manage and process insurance claims and documentation.</p>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl shadow-indigo-900/5 overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50">
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">Claim Pipeline</h2>
            <p className="text-sm font-bold text-slate-400 uppercase tracking-tighter">Review and update active claim requests</p>
          </div>
          <div className="flex items-center space-x-3">
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input type="text" placeholder="Search claims..." className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-600/20 transition-all" />
            </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-6 font-bold text-slate-900">{item.amount}</td>
                  <td className="px-8 py-6">
                    <span className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-tighter ${item.status === 'Approved' ? 'bg-emerald-50 text-emerald-600' : item.status === 'Pending' ? 'bg-amber-50 text-amber-600' : item.status === 'In Progress' ? 'bg-indigo-50 text-indigo-600' : 'bg-rose-50 text-rose-600'}`}>
                      {item.status}
                    </span>
                  </td>
