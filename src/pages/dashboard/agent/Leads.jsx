...
import { 
  ...
  AlertCircle, Star, MessageSquare, X, Plus, User
} from 'lucide-react';
              ...
              <div className="p-0">
                <div className="bg-slate-900 p-8 text-white relative">
                  ...
                  <div className="flex items-center space-x-6">
                    <div className="w-20 h-20 bg-emerald-500 rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-xl shadow-emerald-500/20">
                      {selectedLead.name.charAt(0)}
                    </div>
                    <div>
                      <h2 className="text-2xl font-black tracking-tighter">{selectedLead.name}</h2>
                      <div className="flex items-center space-x-2 mt-1">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest bg-white/10 border border-white/10 ${getStatusColor(selectedLead.status)}`}>
                          {selectedLead.status}
                        </span>
                        <span className="text-slate-400 font-bold text-xs">ID: #{selectedLead.id.toString().slice(-6)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8 space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="font-bold text-slate-900 flex items-center">
                        <Target className="w-3.5 h-3.5 mr-2 text-emerald-500" />
                        {selectedLead.type}
                      </p>
                    </div>
                    <div className="space-y-1">
                      ...
                      <p className="font-bold text-slate-900 flex items-center">
                        <ArrowUpRight className="w-3.5 h-3.5 mr-2 text-blue-500" />
                        {selectedLead.source}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full uppercase tracking-tighter">Last Active: {selectedLead.lastContact}</span>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-center space-x-3 text-xs">
                        <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                        <p className="text-slate-600 font-medium">Initial inquiry received via website form.</p>
                      </div>
                      <div className="flex items-center space-x-3 text-xs opacity-50">
                        <div className="w-2 h-2 bg-slate-300 rounded-full"></div>
                        <p className="text-slate-500 font-medium">Follow-up call scheduled for next Monday.</p>
                      </div>
                    </div>
                  </div>

                  <div className="flex gap-4">
                    <button className="flex-1 px-6 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center space-x-2">
                      <Phone className="w-4 h-4" />
                      <span>Call Now</span>
                    </button>
                    <button className="flex-1 px-6 py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200 flex items-center justify-center space-x-2">
